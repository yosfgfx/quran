...
const MAIN_athkar_JSON_PATH = './assets/data/athkar.json'; // الملف الرئيسي الجديد
    ...
    function renderathkarList(athkarArray, categoryTitle, backButtonAction) {
        ...
        if (athkarArray.length === 0) {
            html += `<div class="text-center p-6 bg-slate-800/50 rounded-lg text-slate-400">لا توجد أذكار متاحة لهذه الفئة حاليًا.</div>`;
             ...
             html += `<div class="text-center text-slate-400 text-sm mb-6"><p>انقر على نص الذكر لتوسيعه أو طيه. انقر على زر "تسبيح" لعد التكرارات.</p></div>`;
                    ...
                    <div class="zikr-card section-card rounded-xl p-5 md:p-6 shadow-lg fade-in">
                        ...
                        <div class="flex flex-col sm:flex-row justify-between items-center gap-3 mt-4 pt-4 border-t border-slate-700">
                            ...
                            <div class="flex items-center gap-2 flex-wrap justify-center sm:justify-end">
                                ${totalCount > 0 ? `
                                <button class="zikr-action-btn zikr-tasbih-btn ${currentCount === 0 ? 'bg-green-600 hover:bg-green-500' : 'bg-amber-600 hover:bg-amber-500'} " 
                                        data-zikr-id="${zikrId}" data-total-count="${totalCount}" title="تسبيح">
                                    ...
                                    <span class="btn-text">${currentCount === 0 ? 'تم' : 'تسبيح'}</span>
                                </button>` : ''}
                                <button class="zikr-action-btn copy-zikr-btn bg-slate-600 hover:bg-slate-500" data-text="${zikr.text}" title="نسخ الذكر">
                                ...
                                <button class="zikr-action-btn share-zikr-btn bg-sky-600 hover:bg-sky-500" data-text="${zikr.text}" title="مشاركة الذكر">
    ...
    function handleTasbihClick(event) {
        ...
        if (currentathkarCounters[zikrId] > 0) {
            ...
            if (btnText) btnText.textContent = 'تسبيح';
